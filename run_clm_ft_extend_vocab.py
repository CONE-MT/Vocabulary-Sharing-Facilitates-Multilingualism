...
#with two tokenizer and two loss
...
def merge_tokenizer(old_tokenizer, vocab_model_path, tokenizer_kwargs=None):
    import os
    ...
    vocab_sp_model = spm.SentencePieceProcessor()
    vocab_sp_model.Load(vocab_model_path)

    old_spm = sp_pb2_model.ModelProto()
    old_spm.ParseFromString(old_tokenizer.sp_model.serialized_model_proto())
    vocab_spm = sp_pb2_model.ModelProto()
    vocab_spm.ParseFromString(vocab_sp_model.serialized_model_proto())  
    
    ## Add new tokens to old tokenizer
    old_spm_tokens_set=set(p.piece for p in old_spm.pieces)
    print(len(old_spm_tokens_set))
    print(f"Before:{len(old_spm_tokens_set)}")
    for p in vocab_spm.pieces:
        piece = p.piece
    ...
    output_sp_dir = os.path.dirname(vocab_model_path)
    output_name = "/merged_"+os.path.basename(vocab_model_path)

    os.makedirs(output_sp_dir,exist_ok=True)
    with open(output_sp_dir+output_name, 'wb') as f:
        f.write(old_spm.SerializeToString())
    tokenizer = LlamaTokenizer(vocab_file=output_sp_dir+output_name,**tokenizer_kwargs)
    return tokenizer
...
class ModelArguments:
    ...
    extend_vocab_file: Optional[str] = field(
        default=None, metadata={"help": "add more tokens"}
    )
...
def main():
    ...
    ############## merge vocabulary
    if model_args.extend_vocab_file is not None:
        extend_vocab_file = model_args.extend_vocab_file + ".model"
        if not os.path.exists(extend_vocab_file):
            raise ValueError("The extend vocabulary file is not found!")
        
        tokenizer = merge_tokenizer(tokenizer, extend_vocab_file,tokenizer_kwargs)

        print(f"Vocabulary merge Finished! Current vocabulary length is {len(tokenizer)}")
