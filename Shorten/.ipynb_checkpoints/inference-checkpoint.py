...
def get_args(parser):
    ...
    parser.add_argument('--decode_map', type=str, default=None, help='model name in the hub or local path')
...
if __name__ == "__main__":
    ...
    if getpass.getuser() == 'root':
        ...
        command = "sudo cp -r /cpfs01/shared/NLP-A100/NLP-A100_hdd/jie/multilingual_LLM/extend/models/* /root/.cache/huggingface/modules/transformers_modules/"  
        os.makedirs("/root/.cache/huggingface/modules/transformers_modules/", exist_ok=True)
        os.system(command)
    else:
    ...
    test_process_func = load_module_from_path(f"{args.project_path}src/tasks/shorten/{config.dataset.loader}_compress.py").test_process
    
    data_dict = data_process_func(config)
    
    map_path = args.decode_map
    test_process_func(config, data_dict, map_path)
    # metric 