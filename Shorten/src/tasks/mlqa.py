...
def test_process(config, data_dict):
    ...
    CONTEXT_LANG, QUESTION_LANG = config.dataset.lang_pair.split("-")
    intput_file_path=os.path.join(config.dataset.path, config.dataset.input_file.replace("CONTEXT_LANG",CONTEXT_LANG).replace("QUESTION_LANG",QUESTION_LANG))
    # metric_file_path=config.metric.metric_json
    answer_lang = config.dataset.lang_pair.split('-')[-1]
    eval_python_script=config.metric.evalution
    if config.metric.python_path:
        command = f"{config.metric.python_path} {eval_python_script} {intput_file_path} {output_file} {answer_lang} {metric_file_path}"
    else:
        command = f"python {eval_python_script} {intput_file_path} {output_file} {answer_lang} {metric_file_path}"
