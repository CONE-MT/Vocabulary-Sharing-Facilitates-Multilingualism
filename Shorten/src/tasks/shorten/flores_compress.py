...
def load_map(path):
      with open(path, 'r', encoding='utf-8') as f:
            mapping = json.load(f)
            outlier = next(iter(mapping.values()))[1][0]
            res = [x[0] for x in mapping.values()]
      return res, outlier
...
def load_model(config):
    ...
    print("Show special tokens in tokenizer",tokenizer.eos_token_id,tokenizer.pad_token_id, tokenizer.eos_token, tokenizer.pad_token)
...
def test_process(config, data_dict, map_path):
    batch_size = config.generat.batch_size
    print(f"Add checkpoint...")
    model, tokenizer, gen_config = load_model(config)
    
    print(f"Begin generating...")
    input_data = data_dict["input_data"]
    prompt = data_dict["prompt_list"]
    input_file=data_dict["input_file"]
    print(f"Prepare output files...")
    token_map, outlier = load_map(map_path)
    def decompress(output_ids):
          new_ids = []
          for output_id in output_ids:
              new_id = []
              for i in range(len(output_id)):
                  if i != len(output_id)-1:
                      pair = [output_id[i],output_id[i+1]]
                      if pair in token_map and new_id[-2]!=outlier:  
                          new_id.append(outlier)
                  new_id.append(output_id[i])
        ...
        for i in tqdm(range(0, len(prompt), batch_size)):
            with torch.autocast("cuda"):
                ...
                with torch.no_grad():
                    ...
                    generated_ids = decompress(generated_ids)
