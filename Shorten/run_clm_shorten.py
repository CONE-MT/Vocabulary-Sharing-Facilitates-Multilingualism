...
class ModelArguments:
    ...
    shorten_id: Optional[int] = field(
        default=None,
        metadata={"help": "the id of token will be shorten"},
    )
...
def main():
    ...
    shorten_id = model_args.shorten_id
    def compress(tokenized):
            input_ids, attention_mask = [],[]
            for ids, mask in zip(tokenized['input_ids'], tokenized['attention_mask']):
                  tmp_id = [x for x in ids if x!=shorten_id]
                  input_ids.append(tmp_id)
                  tmp_mask = mask[:len(tmp_id)]
                  attention_mask.append(tmp_mask)
                  assert len(tmp_id) == len(tmp_mask)
            tokenized['input_ids'], tokenized['attention_mask'] = input_ids, attention_mask
            return tokenized
    ...
    def preprocess_function(examples):
        with CaptureLogger(tok_logger) as cl:
            ...
            if "prefix" in column_names:
                ...
                prefix_tokenized = compress(tokenizer(prefix, truncation=True, max_length=block_size, padding=False))
                text_tokenized = compress(tokenizer(text, truncation=True, max_length=block_size, padding=False))
